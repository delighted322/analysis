...
for (var i = 0; i < jsonFile.length; i++) {
    ...
    if (q.backgroundKnowledge == "G1")
        q.backgroundKnowledge = "G1~G2";
    if (q.backgroundKnowledge == "G2")
        q.backgroundKnowledge = "G1~G2";
    if (q.backgroundKnowledge == "G3")
        q.backgroundKnowledge = "G3~G4";
    if (q.backgroundKnowledge == "G4")
        q.backgroundKnowledge = "G3~G4";
    if (q.backgroundKnowledge == "G5")
        q.backgroundKnowledge = "G5~G6";
    if (q.backgroundKnowledge == "G6")
        q.backgroundKnowledge = "G5~G6";
    ...
    var content = "";
    var countsPerPage = [];
    for (var i_1 = 0; i_1 < contentArray.length; i_1++) {
    ...
    var cOA = utils.getCountOnAverageLevel(totalCount, contentArray.length);
    q.countOnAverage = cOA[0];
    var cPPL = utils.getCountPerPageLevel(contentArray, contentArray.length);
    ...
    // );
    q.lengthLevel = utils.getLengthLevel(contentArray.length, totalCount / contentArray.length);
    if (q.type == "绘本（包含漫画）") {
        // FIXME: 绘本的权重
        ...
        wL[0], // 字词难度
        sL[2], // 句子难度
        q.theme, q.structure, q.languageFeature, q.backgroundKnowledge, 0.25, // 篇幅权重
        0.05, // 不重复字总数权重
        0.1, // 字词难度权重
        0.1, // 句子难度权重,
        0.1, // 主题权重
        ...
        wL[0], // 字词难度
        sL[2], // 句子难度
        q.theme, q.structure, q.languageFeature, q.backgroundKnowledge, 0, // 篇幅权重
        0.1, // 不重复字总数权重
        0.1, // 字词难度权重
        0, // 句子难度权重,
        0.1, // 主题权重
        ...
        wL[0], // 字词难度
        sL[2], // 句子难度
        q.theme, q.structure, q.languageFeature, q.backgroundKnowledge, 0.1, // 篇幅权重
        0.1, // 不重复字总数权重
        0.1, // 字词难度权重
        0, // 句子难度权重,
        0, // 主题权重
